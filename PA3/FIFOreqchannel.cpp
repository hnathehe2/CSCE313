...
FIFORequestChannel::FIFORequestChannel(const string _name, const Side _side) : RequestChannel (_name, _side){
	stream1 = "fifo_" + my_name + "1";
	stream2 = "fifo_" + my_name + "2";
		
	if (_side == SERVER_SIDE){
		wfd = open_ipc(stream1, O_WRONLY | O_CREAT);
		rfd = open_ipc(stream2, O_RDONLY | O_CREAT);
	}
	else{
		rfd = open_ipc(stream1, O_RDONLY | O_CREAT);
		wfd = open_ipc(stream2, O_WRONLY | O_CREAT);
	}
...
FIFORequestChannel::~FIFORequestChannel(){ 
	...
	remove(stream1.c_str());
	remove(stream2.c_str());
}
